...
(TODO: 17:00 during all months, since during winter it is dark?)
```{r fig.width=8, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r fig.width=8, message=FALSE, warning=FALSE, echo=FALSE}
fillcolors <- brewer.pal(11, "Set3")

# t6$CollisionTypeDescriptor <- fct_relevel(t6$CollisionTypeDescriptor,"Fatal Accident", after=Inf)

ggplot(t6, aes(x = factor(Time), y = RelFreq, fill = CollisionTypeDescriptor)) + 
  geom_col() + 
  scale_fill_manual(values = fillcolors) + 
  ggtitle("Collision Type Descriptor of Car Crashes") + 
  labs(x = "Time", y = "Proportion") + 
  theme_classic()
...
h1$`Traffic Control Device` <- h1$`Traffic Control Device` %>%
  fct_relevel("Unknown","None")

fillcolors <- brewer.pal(7, "Set3")

ggplot(h1, aes(x = factor(time_hour), y = RelFreq, fill = `Traffic Control Device`)) + 
  geom_col() + 
  scale_fill_manual(values = fillcolors) + 
  ggtitle("Traffic Control Device of Car Crashes") + 
  labs(x = "Time", y = "Proportion") + 
  theme_classic()
```
...
h2$Collision_Detail <- h2$Collision_Detail %>% 
  # fct_recode(E="Earth Embankment/Rock Cut/Ditch", G="Guide Rail - Not At End", Tree="Tree",
  #            U="Light Support/Utility Pole", Other="Other Fixed Object*", Sign="Sign Post") %>%
  fct_relevel("Other Fixed Object*", "Sign Post", after=Inf)

ggplot(h2, aes(x = factor(time_hour), y = RelFreq, fill = Collision_Detail)) + 
  geom_col() + 
  scale_fill_manual(values = fillcolors) + 
...
# fct_recode(Ani="Animal", Bike="Bicyclist", S="In-Line Skater", Vehicle="Other Motor Vehicle",
